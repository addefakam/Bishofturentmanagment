 ...
 * Optimized: skip indexes if core tables already exist (fast path).
 */
async function ensureSchema(db: PrismaClientInstance) {
  if (schemaReady) return;
  try {
    // ── Phase 1: Create police-specific tables in parallel ──
    await Promise.all([
      db.$executeRawUnsafe(`
    ...
    const pSet = new Set(providerCols.map(c => c.name));
    const uSet = new Set(userCols.map(c => c.name));
    ...
    // ── Phase 4: Create indexes ONLY if any table was just created ──
    // Skip indexes on warm starts when all tables already exist (massive latency savings)
    const indexesNeeded = !pSet.size || pSet.size < 5; // Provider table has <5 cols = fresh DB
    if (indexesNeeded) {
      const idx = [
      ...
      // Batch of 15 for faster parallel execution on fresh DB
      for (let i = 0; i < idx.length; i += 15) {
        await Promise.all(
          idx.slice(i, i + 15).map(s =>
            db.$executeRawUnsafe(s).catch(e => console.warn("[db] Index skipped:", (e as Error).message))
          )
        );
      }
      console.log(`[db] Created ${idx.length} indexes (fresh database)`);
    } else {
      console.log("[db] Skipping indexes — tables already exist");
    }
